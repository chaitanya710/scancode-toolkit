...
class ModelMixin:
    ...
    @classmethod
    def merge_sequences(cls, list1, list2, **kwargs):
        """
...
class IdentifiablePackageData(ModelMixin):
    ...
    def to_dict(self, **kwargs):
        mapping = super(IdentifiablePackageData, self).to_dict(**kwargs)
...
@attr.attributes(slots=True)
class BaseFile(ModelMixin):
    path = String(
        label='Path of this file.',
        help='The file or directory POSIX scanned path.',
        repr=True,
    )
...
class File(BaseFile):
    ...
    def to_dict(self, with_details=True, **kwargs):
        mapping = super(File, self).to_dict(with_details=with_details, **kwargs)
        if not with_details:
            del mapping['is_file']
        return mapping
...
class PackageData(IdentifiablePackageData):
    ...
    files = List(
        ...
        help='List of package-provided resources (with file path) that exist in the codebase.'
    )
    ...
    def to_dict(self, with_details=True, **kwargs):
        mapping = super(PackageData, self).to_dict(with_details=with_details, **kwargs)
        if not with_details:
            del mapping['files']
            del mapping['dependencies']
            del mapping['datasource_ids']

        return mapping

    @classmethod
    def from_dict(cls, scan_data):
        """
        ...
        for name, value in scan_data.items():
            if not value:
                continue
        ...
        return super(PackageData, self).from_dict(package_data)
...
class DatafileHandler:
    ...
    def is_datafile(cls, location):
        ...
        that this parser can handle. This basic implementation should work in
        most cases and is based on matching as a whole any one of the path
        patterns. The path patterns are for POSIX paths.

        Subclasses can override to implement more complex data file recognition.
        """
        if filetype.is_file(location):
            loc = as_posixpath(location)
            return any(fnmatch(loc, pat) for pat in cls.path_patterns)
    ...
    def compute_normalized_license(cls, package, resource, codebase, expression_symbols=None):
        ...
        """
        if package and not package.license_expression:
            try:
    ...
    def assign_package_to_resources(cls, package, resource, codebase):
        """
        Set the "for_packages" attributes to ``package`` given a ``resource`` in
        the ``codebase``.

        This default implementation only assigns the datafile ``resource`` to
        this package.
        ...
        resource.for_packages.append(package.package_uid)
    ...
    def assign_package_to_parent_tree(cls, package, resource, codebase):
        ...
        tree of the parent of a ``resource`` object in the ``codebase``.
        Convenience method that subclass can use.
        """
        package_uid = package.package_uid
        parent = resource.parent(codebase)
        for res in parent.walk(codebase):
...
class Package(PackageData):
    ...
    def to_package_data(self):
        """
        Return a PackageData object from this Package.
        """
        mapping = self.to_dict()

        # Removes Package specific attributes
        for attribute in ('package_uid', 'datafiles', 'datasource_ids'):
    ...
    def from_package_data(cls, package_data, datafile_path):
        ...
        if isinstance(package_data, PackageData):
            package_data = package_data.to_dict()
        else:
            # make a copy
            package_data = dict(package_data)

        package_data['datafile_paths'] = [datafile_path]
        package_data['datasource_ids'] = [package_data['datasource_id']]
    ...
    def update(self, package_data, replace=False):
        ...
        list_fields = {'parties', 'dependencies', 'files', 'file_references'}
        ...
        for name, value in existing.items():
            ...
            # here we do not replace... but we still merge lists
            if name in list_fields:
                if TRACE_UPDATE: logger_debug('  merge lists of values')
                merged = self.merge_sequences(list1=value, list2=new_value)
