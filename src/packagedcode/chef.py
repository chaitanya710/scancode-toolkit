...
class ChefCookbookHandler(models.PackageData):
    ...
    # TODO: extract and parse and register
...
class ChecfMetadataJsonHandler(models.DatafileHandler):
    ...
    def is_datafile(cls, location):
        ...
        directory.
        """
        ipdf = super(ChecfMetadataJsonHandler, cls).is_datafile(location)
        if ipdf:
...
class ChefMetadatarbHandler(models.DatafileHandler):
...
def build_package(package_data, datasource_id):
    """
    Return a PackageData object from a package_data mapping (from a
    metadata.json or similar) or None.
    """
    ...
    yield models.PackageData(
        ...
        type=ChecfMetadataJsonHandler.default_package_type,
        name=name,
        version=version,
        parties=parties,
