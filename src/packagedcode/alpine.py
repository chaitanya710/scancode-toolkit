...
class AlpineApkArchiveHandler(models.DatafileHandler, BaseAlpineHandlerMixin):
    ...
    def is_datafile(cls, location):
        ipdf = super(AlpineApkArchiveHandler, cls).is_datafile(location)
        if ipdf:
            T = contenttype.get_type(location)
            return 'gzip compressed data' in T.filetype_file.lower()


class AlpineInstalledDatabaseHandler(models.DatafileHandler, BaseAlpineHandlerMixin):
...
class AlpineApkbuildHandler(models.DatafileHandler, BaseAlpineHandlerMixin):
...
def get_installed_packages(root_dir, **kwargs):
    ...
    if path.exists(installed_file_loc):
...
def get_alpine_installed_db_fields(location):
    """
    Yield a list of (name, value) pairs for each package found in an installed
    Alpine packages database file at `location` (typically found at
...
def build_package(package_fields, datasource_id, package_type):
    """
    ...
    converted_fields['type'] = package_type
    converted_fields['datasource_id'] = datasource_id
